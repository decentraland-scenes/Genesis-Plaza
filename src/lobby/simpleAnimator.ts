...
export class ItemAnimationSystem {
  ...
  update(dt: number) {
    for (let item of this.group.entities) {
      ...
      if (info.isHighlighted) {
        transform.scale = this.springVec3(
          info.highlightTransform.scale,
          transform.scale,
          info.animVeclocity,
          info.speed * dt
        )
        transform.position = this.springVec3(
  ...
  springVec3(
    ...
    return a_Current.add(
      currentToTargetVec
        .normalize()
        .multiplyByFloats(displacement, displacement, displacement)
    )
  }
}
