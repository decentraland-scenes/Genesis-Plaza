...
export class CooldownActivated {
  ...
  constructor(
    ...
    _originalHoverText: string,
    _inactiveHoverText: string
  ) {
    ...
    for (let item of this.group.entities) {
      ...
      if (cooldownInfo.elapsed < cooldownInfo.cooldownLength) {
        ...
        if (item.hasComponent(TextShape)) {
          ...
          item.getComponent(TextShape).value = Math.ceil(
            cooldownInfo.cooldownLength - cooldownInfo.elapsed
          ).toString()
        }

        if (item.hasComponent(OnPointerDown)) {
          item.getComponent(OnPointerDown).hoverText =
            cooldownInfo.inactiveHoverText
        }
      } else {
        ...
        if (item.hasComponent(OnPointerDown)) {
          item.getComponent(OnPointerDown).hoverText =
            cooldownInfo.originalHoverText
        }
      }
    }
  }
