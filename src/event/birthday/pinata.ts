...
import { handlePoap, sendpoap } from './poapHandler'
...
pinataMain.addComponent(
  new OnPointerDown(
    () => {
      isHit = true
      chainAmp = 45
      pinataAmp = 30
      vectorToPlayer = player.position
        .subtract(pinataFrame.getComponent(Transform).position)
...
class PinataSystem {
  ...
  update(dt: number) {
    if (isHit) {
      ...
      chainTransform.rotation = Quaternion.RotationAxis(
        rotateAxis,
        ToDegrees(Math.sin(this.elapsedChainTime * 3) * chainAmp)
      )
      pinataTransform.rotation = Quaternion.RotationAxis(
        rotateAxis,
        ToDegrees(Math.sin(this.elapsedPinataTime * 6) * pinataAmp)
      )
    ...
    confetti1.addComponent(
      new Transform({
        position: new Vector3(
          pinataTransform.position.x + Math.random(),
          1 + Math.random() * 5,
          pinataTransform.position.z + Math.random()
        ),
